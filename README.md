...
**ВНИМАНИЕ! Часть функционала SDK такая как закрытие фрейма после оплаты или возврат в приложение работает только в боевом режиме - поэтому не рекомендуется использовать isTesting=true.**
...
Процесс проверки статуса платежа состоит из следующих этапов:
